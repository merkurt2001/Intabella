...
Feature: Vehicle General Information
  ...
  @wip
    Scenario Outline: User can see the General Information Page by clicking on the Eye icon
      When the "<userType>" enters the username "<username>" and password "<password>" information
      And the user click on the "Fleet" tab "Vehicles" module as user "<userType>"
      And the user can see the General Information page by clicking the Eye icon "<userType>"
      Examples:
        | userType      | username        | password    |
        | driver        | user1           | UserUser123 |
        | sales manager | salesmanager103 | UserUser123 |
    ...
    Scenario Outline: Manager's should see Delete Edit and Add Event on the General Information page
      When the "<userType>" enters the username "<username>" and password "<password>" information
      And the user click on the "Fleet" tab "Vehicles" module as user "<userType>"
      When the "<userType>" clicks on any vehicle row and he lands to the General Information page he should see the Delete Edit and Add Event buttons
      Examples:
        | userType      | username        | password    |
        | sales manager | salesmanager103 | UserUser123 |
        | store manager | storemanager85  | UserUser123 |
    ...
    Scenario Outline: Information should be the same as on General Information and Fleet-Vehicle page
      When the "<userType>" enters the username "<username>" and password "<password>" information
      And the user click on the "Fleet" tab "Vehicles" module as user "<userType>"
